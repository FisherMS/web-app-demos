...
<template>
  <nav class="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-md border-t border-gray-100/50 z-50 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)]">
    <div class="flex justify-around items-center h-16 max-w-md mx-auto">
      <router-link
        ...
        :class="normalizedPath === link.href ? 'text-black' : 'text-gray-400 hover:text-gray-600'"
      >
        <div
          class="mb-1 transition-transform duration-300"
          :class="normalizedPath === link.href ? 'scale-110' : 'group-hover:scale-110'"
          v-html="link.icon"
        ></div>
        <span
          class="text-[10px] font-bold tracking-tight"
          :class="normalizedPath === link.href ? 'opacity-100' : 'opacity-80'"
        >
          {{ link.label }}
        </span>
      </router-link>
