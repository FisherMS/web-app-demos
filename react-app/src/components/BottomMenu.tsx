...
/**
 * BottomMenu component summary:
 * Provides a fixed navigation menu at the bottom of the screen, typical for mobile-first web apps.
...
const BottomMenu: React.FC = () => {
    ...
    return (
        <nav
            className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-md border-t border-gray-100/50 z-50 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)]"
        >
                ...
                {links.map((link) => {
                    ...
                    return (
                        <Link
                            key={link.href}
                            to={link.href}
                            className={`flex flex-col items-center justify-center w-full h-full transition-all duration-300 group ${isActive ? "text-black" : "text-gray-400 hover:text-gray-600"}`}
                        >
                            <div
                            ...
                            <span
                                className={`text-[10px] font-bold tracking-tight ${isActive ? "opacity-100" : "opacity-80"}`}
                            >
                                {link.label}
                            </span>
                        </Link>
